...
得到的 JSON 配置文件（`foo.cliq.json`）应该放在自己项目的根目录中。

### 第二步：集成 cliq

克隆本项目到你的仓库中，在本项目文件夹中运行：

> 如果您使用 deno ...

```bash
deno install -A
deno task tauri dev  # or build, ...
```
...
```bash
npm install
npm run tauri dev  # or build, ...
```

> 如果您使用 [mise](https://github.com/jdx/mise) ...
